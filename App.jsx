...
const greetingFiller = "Discover our exclusive selection of 20 unique wines, chosen for you."
...
function App() {
    ...
    const addToCart = (wine, quantity) => {
        setCartItems(prevItems => {
            ...
            if (existingItem) {
                return prevItems.map(item => item.id === wine.id ? { ...item, quantity: item.quantity + quantity } : item)
            }else{
    ...
    return(
        ...
        <CartProvider>
            <Layout setSelectedCategory={setSelectedCategory} >
                <Routes> 
                    <Route path="/" element={<ItemsContainer selectedCategory={selectedCategory} placeholder={"Search wines..."} greeting={"Welcome!"} greetingFiller={`${greetingFiller}`} />} />
