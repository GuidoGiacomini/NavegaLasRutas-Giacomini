...
const Content = ({ selectedCategory }) =>{
    ...
    const fetchWinesFromFirebase = async () => {
        try {
            ...
            const winesData = querySnapshot.docs.map((doc) => ({id: doc.id, ...doc.data()}))
            setWines(winesData)
        } catch (error) {
    ...
    const filteredWines = selectedCategory ? wines.filter((wine) => wine.category === selectedCategory) : wines

    return(
    <>
        <ul className="content">
            {filteredWines.length === 0 ? (<p style={{fontSize : '40px', color : 'white'}}>Collecting bottles...</p>) : 
